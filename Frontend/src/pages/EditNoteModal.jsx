        ...
        >
          ...
          >
            <div className="flex justify-between items-center mb-4">
              <h2 className="font-bold text-lg text-slate-900 dark:text-white">Edit Note</h2>
              <button
                onClick={onClose}
                className="text-slate-400 dark:text-slate-500 hover:text-slate-700 dark:hover:text-white cursor-pointer"
              >
                <LuX size={18} />
              </button>
            </div>
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-3.5">
              <input
                ...
                rounded-xl px-4 py-2.5 text-xs font-semibold outline-none 
                focus:border-slate-900 dark:focus:border-slate-400 shadow-2xs"
              />
              <textarea
                value={body}
                ...
                className="border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 
                text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-500 
                rounded-xl px-4 py-2.5 text-xs font-medium outline-none 
                focus:border-slate-900 dark:focus:border-slate-400 shadow-2xs resize-none"
              />
              ...
              {imagePreview ? (
                <div className="relative rounded-2xl overflow-hidden border border-slate-200 dark:border-slate-700 group">
                  ...
                  <button
                    type="button"
                    onClick={removeImage}
                    className="absolute top-2 right-2 bg-black/70 hover:bg-black text-white 
                ...
                text-slate-500 dark:text-slate-400 hover:border-slate-800 dark:hover:border-slate-400 
                hover:text-slate-800 dark:hover:text-white transition-colors cursor-pointer 
              ...
              <label className="flex items-center gap-2 text-xs font-medium text-slate-600 dark:text-slate-300 mt-1 cursor-pointer">
                <input
                  type="checkbox"
                  checked={isPublic}
                  onChange={(e) => setIsPublic(e.target.checked)}
                  className="rounded text-slate-900 dark:text-white dark:bg-slate-800 
                  dark:border-slate-600 focus:ring-0 cursor-pointer"
                />
