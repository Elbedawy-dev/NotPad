...
const Profile = () => {
  const { user, setUser, updateProfile, deleteAccount } =
    useContext(AuthContext);
  const navigate = useNavigate();
  const { t } = useApp();

  const [fullName, setFullName] = useState(user?.name || "moaaz Jenkins");
  const [selectedAvatar, setSelectedAvatar] = useState(
    user?.avatar || "/avatars/1.png",
  );
            ...
            <div className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-100 dark:border-slate-800 p-8 shadow-xs">
              ...
              <div className="flex flex-col items-center mb-8">
                ...
                <div className="flex items-center gap-1.5 text-xs text-slate-400 dark:text-slate-500 font-medium">
                  ...
                  <span>{user?.email || "moaaz.j@minimal.io"}</span>
                </div>
              </div>
              ...
              <form onSubmit={handleSaveChanges} className="space-y-5">
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <LuPencil
                      size={15}
                      className="absolute ltr:right-3.5 rtl:left-3.5 top-1/2 -translate-y-1/2 text-slate-400 dark:text-slate-500"
                    />
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="relative">
                    <input
                      type="email"
                      value={user?.email || "moaaz.j@minimal.io"}
                      disabled
                  ...
                  <button
                    ...
                    className={`rounded-full overflow-hidden border-2 transition-transform hover:scale-105 ${
                      selectedAvatar === avatar
                        ? "border-slate-950 dark:border-white ring-2 ring-slate-950/20 dark:ring-white/20"
                        : "border-transparent"
                    }`}
                  >
                    <img
