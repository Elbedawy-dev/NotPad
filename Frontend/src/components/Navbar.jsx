...
import { LuNotebookPen, LuLogOut, LuLayoutGrid, LuUser, LuSun, LuMoon, LuLanguages, LuMenu, LuX } from "react-icons/lu"
...
const Navbar = ({ notesCount = 0, activeTab = "all", onTabChange }) => {
  ...
  return (
    <header className="w-full bg-white dark:bg-slate-900 border-b border-slate-100 dark:border-slate-800 px-4 sm:px-6 py-3 sticky top-0 z-40 transition-colors">
      <div className="max-w-7xl mx-auto flex items-center justify-between">
        
        {/* Left Section: Logo, Title, Badge, Saved Count, Desktop Nav Tabs */}
        <div className="flex items-center gap-3 sm:gap-6">
          <Link to="/" className="flex items-center gap-2 sm:gap-2.5 group">
            <div className="w-8 h-8 sm:w-9 sm:h-9 bg-black dark:bg-slate-100 rounded-xl flex items-center justify-center shadow-sm group-hover:scale-105 transition-transform">
              <LuNotebookPen className="text-white dark:text-slate-900 text-base sm:text-lg" />
            </div>
            <span className="font-bold text-lg sm:text-xl text-slate-900 dark:text-white tracking-tight">{t("notes")}</span>
          </Link>

          <span className={`hidden sm:inline-flex text-xs font-medium px-2.5 py-1 rounded-full border transition-colors ${
            isDashboard 
            ...
            <div className="flex items-center gap-3">
              ...
              >
                ...
                <div className="hidden lg:block">
                  <p className="text-xs font-bold text-slate-900 dark:text-white leading-tight">{user?.name || "Sarah Jenkins"}</p>
                  <p className="text-[11px] text-slate-400 dark:text-slate-500 leading-tight">{user?.email || "sarah.j@minimal.io"}</p>
                </div>
              </button>
              ...
              <div className="absolute right-0 mt-2 w-64 bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-2xl shadow-xl p-2 z-50 animate-in fade-in duration-150">
                ...
                <div className="space-y-0.5 pt-1">
                  ...
                  >
                    ...
                    {isHome && <span className="w-1.5 h-1.5 rounded-full bg-slate-900 dark:bg-white"></span>}
                  </button>
                  ...
                  >
                    ...
                    <div className="flex items-center gap-1.5">
                      <span className="text-[10px] text-slate-400 bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 px-1.5 py-0.5 rounded font-mono">⌘D</span>
                      {isDashboard && <span className="w-1.5 h-1.5 rounded-full bg-slate-900 dark:bg-white"></span>}
                    </div>
                  </button>
                  ...
                  >
                    ...
                    {isProfile && <span className="w-1.5 h-1.5 rounded-full bg-slate-900 dark:bg-white"></span>}
                  </button>
      ...
      {mobileMenuOpen && (
        <div className="md:hidden pt-3 mt-3 border-t border-slate-100 dark:border-slate-800 space-y-4 animate-in slide-in-from-top-2 duration-200">
          ...
          <div className="pt-2 border-t border-slate-100 dark:border-slate-800">
            <div className="flex items-center justify-between p-3 bg-slate-50 dark:bg-slate-800/60 rounded-xl mb-2 border border-slate-100 dark:border-slate-800">
              <div className="flex items-center gap-3 overflow-hidden">
                ...
                />
                <div className="overflow-hidden">
                  <p className="text-xs font-bold text-slate-900 dark:text-white truncate">{user?.name || "Sarah Jenkins"}</p>
                  <p className="text-[11px] text-slate-400 dark:text-slate-500 truncate">{user?.email || "sarah.j@minimal.io"}</p>
                </div>
              </div>
